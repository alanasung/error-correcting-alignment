# Design Document: Active Correction Loops for Alignment Drift

## 1. Purpose

This document is the method contract for **Active Correction Loops for Alignment Drift**. It specifies what is
...
One-liner: Pilot an error-correction style loop that detects and repairs alignment drift on toy tasks.

Hypothesis: Pilot an error-correction style loop that detects and repairs alignment drift on toy tasks.
...
|---|---|---|
| Config schema | `realign.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `realign.configs.loader` | Hydra compose API |
| Device | `realign.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `realign.models.registry` | Pinned revisions + hardware fit |
| Hooks | `realign.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `realign.models.generation` | Chat template path recording |
| Data | `realign.data` | Synthetic items, splits, manifests |
| Evaluation | `realign.evaluation` | Metrics, failure harness, protocols |
| Ablation | `realign.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `realign.reporting` | Aggregate → MD/TeX/figures |
| Cache | `realign.cache` | Versioned artifact store |
...
`src/realign/realign/` are preserved across regeneration.
